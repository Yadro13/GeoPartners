...
export async function exportResultReportDocx(groups: ResultReportGroup[], type: PlotResultType, requestedLocale: string = defaultLocale, statuses: PlotStatusDefinition[] = [], includeExpenses = true) {
  ...
  for (const group of groups) {
    ...
    children.push(new Table({ width: { size: tableWidth, type: WidthType.DXA }, columnWidths: infoWidths, layout: TableLayoutType.AUTOFIT, rows: [new TableRow({ children: [cell(labels.relationship, true, infoWidths[0]), cell(labels.cadastralNumber, true, infoWidths[1]), cell(`${labels.owner} / ${labels.lessee}`, true, infoWidths[2]), cell(labels.specialParameters, true, infoWidths[3])] }), new TableRow({ children: [cell(labels.mainCandidate, false, infoWidths[0]), cell(primary?.cadastralNumber ?? labels.notSpecified, false, infoWidths[1]), cell(primary ? `${primary.owner}\n${primary.lessee}` : labels.notSpecified, false, infoWidths[2]), cell(primary ? formatPlotSpecialDetails(primary, type, locale, false) : labels.notSpecified, false, infoWidths[3])] }), ...group.alternativeCandidates.map(({ properties }) => new TableRow({ children: [cell(labels.alternativeCandidate, false, infoWidths[0]), cell(properties.cadastralNumber, false, infoWidths[1]), cell(`${properties.owner}\n${properties.lessee}`, false, infoWidths[2]), cell(formatPlotSpecialDetails(properties, type, locale, false), false, infoWidths[3])] })), ...group.finalPlots.map(({ properties }) => new TableRow({ children: [cell(resultLabels.finalPlot, false, infoWidths[0]), cell(properties.cadastralNumber, false, infoWidths[1]), cell(`${properties.owner}\n${properties.lessee}`, false, infoWidths[2]), cell(formatPlotSpecialDetails(properties, type, locale, true), false, infoWidths[3])] }))] }));
...
function resultPdfBlock(group: ResultReportGroup, statuses: PlotStatusDefinition[], locale: AppLocale, labels: (typeof reportLabels)[AppLocale], resultLabels: { title: string; number: string; finalPlot: string }, includeExpenses: boolean): Content[] {
  const primary = group.primary?.properties;
  const rows = [[labels.relationship, labels.cadastralNumber, `${labels.owner} / ${labels.lessee}`, labels.specialParameters], [labels.mainCandidate, primary?.cadastralNumber ?? labels.notSpecified, primary ? `${primary.owner}\n${primary.lessee}` : labels.notSpecified, primary ? formatPlotSpecialDetails(primary, group.type, locale, false) : labels.notSpecified], ...group.alternativeCandidates.map(({ properties }) => [labels.alternativeCandidate, properties.cadastralNumber, `${properties.owner}\n${properties.lessee}`, formatPlotSpecialDetails(properties, group.type, locale, false)]), ...group.finalPlots.map(({ properties }) => [resultLabels.finalPlot, properties.cadastralNumber, `${properties.owner}\n${properties.lessee}`, formatPlotSpecialDetails(properties, group.type, locale, true)])];
