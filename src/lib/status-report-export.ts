...
export async function exportResultReportXlsx(groups: ResultReportGroup[], type: PlotResultType, statuses: PlotStatusDefinition[], requestedLocale: string = defaultLocale, categories: Record<string, CategoryDefinition> = {}, includeExpenses = true) {
  ...
  candidates.columns = [{ header: resultLabels.number, key: "result", width: 14 }, { header: labels.relationship, key: "relationship", width: 22 }, { header: labels.cadastralNumber, key: "cadastral", width: 25 }, { header: labels.category, key: "category", width: 22 }, { header: labels.owner, key: "owner", width: 34 }, { header: labels.lessee, key: "lessee", width: 34 }, { header: labels.specialParameters, key: "parameters", width: 38 }];
  ...
  for (const group of groups) for (const plot of resultGroupPlots(group)) {
    const isFinal = group.finalPlots.some(({ properties }) => properties.id === plot.properties.id);
    const relationship = group.mainCandidates.some(({ properties }) => properties.id === plot.properties.id) ? labels.mainCandidate : group.alternativeCandidates.some(({ properties }) => properties.id === plot.properties.id) ? labels.alternativeCandidate : isFinal ? resultLabels.finalPlot : labels.relatedPlot;
    const row = candidates.addRow({ result: group.number, relationship, cadastral: plot.properties.cadastralNumber, category: categories[plot.properties.category]?.name ?? plot.properties.category, owner: plot.properties.owner, lessee: plot.properties.lessee, parameters: formatPlotSpecialDetails(plot.properties, type, locale, isFinal) }); row.alignment = { vertical: "top", wrapText: true };
  }
  candidates.autoFilter = { from: "A1", to: `G${Math.max(1, candidates.rowCount)}` }; candidates.eachRow((row) => row.eachCell({ includeEmpty: true }, (cell) => { cell.border = thinBorder(); }));
