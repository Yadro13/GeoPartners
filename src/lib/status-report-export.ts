...
export async function exportResultReportXlsx(groups: ResultReportGroup[], type: PlotResultType, statuses: PlotStatusDefinition[], requestedLocale: string = defaultLocale, categories: Record<string, CategoryDefinition> = {}, includeExpenses = true) {
  ...
  for (const group of groups) for (const plot of resultGroupPlots(group)) {
    ...
    const row = candidates.addRow({ result: group.number, relationship, cadastral: plot.properties.cadastralNumber, category: categories[plot.properties.category]?.name ?? plot.properties.category, owner: plot.properties.owner, lessee: plot.properties.lessee, parameters: formatPlotSpecialDetails(plot.properties, type, locale) }); row.alignment = { vertical: "top", wrapText: true };
  }
