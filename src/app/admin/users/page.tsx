...
export default async function AdminUsersPage() {
  ...
  return <main className="admin-shell"><header className="admin-header"><div className="brand-lockup"><span className="brand-mark">GP</span><strong>GeoPartners</strong></div><div className="admin-header__actions"><LanguageSwitcher compact /><Link href="/"><ArrowLeft size={16} />{t("toMap")}</Link></div></header><section className="admin-content admin-content--wide"><div className="admin-title"><span className="eyebrow">{t("administration")}</span><h1>{t("users")}</h1></div><UserManagementTable initialUsers={users.map(({ account, registrationRequestId }) => ({ id: account.id, name: account.name, email: account.email, role: account.role, accessLevel: account.role === "admin" ? "edit" : account.accessLevel, approvalStatus: account.approvalStatus, registrationMethod: account.registrationMethod, registrationRequestId, createdAt: account.createdAt.toISOString(), protected: account.id === admin.id || account.email.toLocaleLowerCase() === protectedEmail }))} /></section></main>;
}
