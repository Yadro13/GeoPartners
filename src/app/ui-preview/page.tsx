...
export default async function UiPreviewPage({ searchParams }: { searchParams: Promise<{ role?: string; access?: string; google?: string; view?: string }> }) {
  ...
  if (view === "users") {
    return <Workspace preview initialSection="users" initialUsers={previewUsers} />;
  }
  ...
  return <Workspace preview initialUsers={previewUsers} googleEnabled={google === "1"} user={role === "user" ? { name: "Демо Користувач", email: "user@example.com", role: "user", accessLevel: access === "edit" ? "edit" : "read" } : undefined} />;
}
