...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const before = plotRowToFeature(current);
    if (!hasPermission(currentUser, "expenses.manage")) feature = preservePlotExpenses(feature, before);
    ...
    const changes = changedPlotFields(before, feature);
    ...
    return NextResponse.json(plotForExpenseAccess(feature, hasPermission(currentUser, "expenses.view")));
