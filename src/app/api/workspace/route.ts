...
export async function GET() {
  ...
  return NextResponse.json(await getWorkspaceContext(currentUser.preferredWorkspace));
}
...
export async function POST(request: Request) {
  ...
  const context = await getWorkspaceContext(currentUser.preferredWorkspace);
  ...
  await db.update(user).set({ preferredWorkspace: parsed.data.workspace }).where(eq(user.id, currentUser.id));
