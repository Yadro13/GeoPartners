...
import { applySafeGeometryRepairs, buildReviewedImportFiles, inspectImportPackage, setAllImportCandidatesIncluded, setImportCandidateIncluded, type ImportIssue, type ImportReview } from "@/lib/import-review";
...
export function ImportForm({ onImport, onCancel, existingPlots, categories, baseMap }: { onImport: (files: File[]) => Promise<void>; onCancel: () => void; existingPlots: PlotFeature[]; categories: Record<string, CategoryDefinition>; baseMap: BaseMapId }) {
  ...
  const analyze = async () => { setError(""); setLoading(true); try { if (!geoCount) throw new Error(t("addGeoJson")); const result = await inspectImportPackage(files, existingPlots, categories); setReview(result); setRepairBackup(null); setSelectedId(result.candidates[0]?.plot.properties.id ?? null); } catch (reason) { setError(reason instanceof Error ? reason.message : t("analyzeFailed")); } finally { setLoading(false); } };
  const confirm = async () => { if (!review) return; setError(""); setLoading(true); try { await onImport(buildReviewedImportFiles(review, files)); } catch (reason) { setError(reason instanceof Error ? reason.message : t("importFailed")); setLoading(false); } };
...
type ImportKey = "bytes" | "kilobytes" | "geometryErrors" | "geometryWarnings" | "geometryValid" | "closeRings" | "removeDuplicates" | "squareMeters" | "betweenFiles" | "withDatabase" | "pair" | "pairs" | "issueObjectSkipped" | "issueCadMismatch" | "issuePdfMissing" | "issueInvalidCad" | "issueGeoRead" | "issuePdfUnmatched" | "issueNoPlots" | "issueDuplicateCad" | "issueRing" | "issueCoordinates" | "issueDuplicateVertices" | "issueSelfIntersection" | "issueOgc" | "issueShortSegment" | "issueOverlap" | "microOverlap" | "overlap" | "sourcePackage" | "sourceDatabase" | "issueUnknown";
...
function localizeImportIssue(issue: ImportIssue, locale: string, t: ImportTranslator, formatNumber: NumberFormatter) {
  ...
  const simpleKeys: Record<string, ImportKey> = {
    "object-skipped": "issueObjectSkipped", "cad-mismatch": "issueCadMismatch", "pdf-missing": "issuePdfMissing", "invalid-cad": "issueInvalidCad", "geo-read": "issueGeoRead", "pdf-unmatched": "issuePdfUnmatched", "no-plots": "issueNoPlots", "duplicate-cad": "issueDuplicateCad", "geometry-ring": "issueRing", "geometry-coordinates": "issueCoordinates", "geometry-duplicate": "issueDuplicateVertices", "geometry-self-intersection": "issueSelfIntersection", "geometry-ogc": "issueOgc", "geometry-short-segment": "issueShortSegment",
  };
