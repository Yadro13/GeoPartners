...
export function PlotForm({ plot, neighbors, categories, plotStatuses, baseMap, onSave, onDelete, onCancel }: { plot: PlotFeature | null; neighbors: PlotFeature[]; categories: Record<string, CategoryDefinition>; plotStatuses: PlotStatusDefinition[]; baseMap: BaseMapId; onSave: (plot: PlotFeature) => Promise<void>; onDelete?: () => void; onCancel: () => void }) {
  ...
  return <form className="plot-form" onSubmit={submit}><div className="form-grid">
    ...
    <label>Проєктна потужність, МВт<input name="projectCapacity" type="number" min="0" step="0.01" defaultValue={initial.projectCapacity} /></label><label>Статус<select name="status" defaultValue={initial.status}><option value="">Без статусу</option>{initial.status && !plotStatuses.some(({ name }) => name === initial.status) ? <option value={initial.status}>{initial.status} (поза довідником)</option> : null}{plotStatuses.map(({ id, name }) => <option key={id} value={name}>{name}</option>)}</select></label>
