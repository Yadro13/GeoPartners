...
export function PlotForm({ plot, neighbors, categories, baseMap, onSave, onDelete, onCancel }: { plot: PlotFeature | null; neighbors: PlotFeature[]; categories: Record<string, CategoryDefinition>; baseMap: BaseMapId; onSave: (plot: PlotFeature) => Promise<void>; onDelete?: () => void; onCancel: () => void }) {
  ...
  const initial = useMemo(() => plot?.properties ?? { id: crypto.randomUUID(), cadastralNumber: "", name: "", category: "default", areaHa: 0, projectCapacity: 0, mainCandidateCadastral: "", owner: "", lessee: "", status: "", statusProgress: [] }, [plot]);
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const next: PlotFeature = { type: "Feature", geometry: parsed, properties: { ...initial, cadastralNumber: String(data.get("cadastralNumber") ?? "").trim(), name: String(data.get("name") ?? "").trim(), category: String(data.get("category") ?? "default"), areaHa: Number(data.get("areaHa")) || 0, projectCapacity: Number(data.get("projectCapacity")) || 0, mainCandidateCadastral: String(data.get("mainCandidateCadastral") ?? "").trim(), owner: String(data.get("owner") ?? "").trim(), lessee: String(data.get("lessee") ?? "").trim() } };
  ...
  return <form className="plot-form" onSubmit={submit}><div className="form-grid">
    ...
    <label className="form-grid__wide">{t("mainCandidate")}<input name="mainCandidateCadastral" defaultValue={initial.mainCandidateCadastral} /></label><label className="form-grid__wide">{t("owner")}<input name="owner" defaultValue={initial.owner} /></label><label className="form-grid__wide">{t("lessee")}<input name="lessee" defaultValue={initial.lessee} /></label>
