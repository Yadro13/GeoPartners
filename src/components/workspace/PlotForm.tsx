...
export function PlotForm({ plot, neighbors, categories, baseMap, onSave, onDelete, onCancel }: { plot: PlotFeature | null; neighbors: PlotFeature[]; categories: Record<string, CategoryDefinition>; baseMap: BaseMapId; onSave: (plot: PlotFeature) => Promise<void>; onDelete?: () => void; onCancel: () => void }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const currentConflicts = findPlotConflicts(parsed, neighbors);
      if (currentConflicts.length) throw new Error(`Контур накладається на ${currentConflicts.length} сусідні ділянки.`);
  ...
  return <form className="plot-form" onSubmit={submit}><div className="form-grid">
    ...
    <div className="form-grid__wide"><GeometryEditor geometry={geometry} initialGeometry={initialGeometry} neighbors={neighbors} conflicts={conflicts} baseMap={baseMap} onChange={updateGeometry} />{conflicts.length ? <div className="geometry-conflicts" role="alert"><AlertTriangle size={19} /><div><strong>Виявлено накладання</strong>{conflicts.map((conflict) => <span key={conflict.plotId}>{conflict.cadastralNumber} · {formatOverlapArea(conflict.overlapAreaHa)}</span>)}</div></div> : null}</div>
