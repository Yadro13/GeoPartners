...
export function SettingsPanel({ actions, canImport, workspace, testWorkspaceEnabled, canManageWorkspaces }: { actions: WorkspaceActions; canImport: boolean; workspace: DataWorkspace; testWorkspaceEnabled: boolean; canManageWorkspaces: boolean }) {
  return <section className="workspace-page"><header className="workspace-page__header"><div><span className="eyebrow">Обмін даними</span><h1>Налаштування</h1></div></header><div className="settings-actions">{canImport ? <button className="command-button command-button--primary" type="button" onClick={actions.openImport}><FileJson size={18} />Імпортувати GeoJSON</button> : null}<button className="command-button" type="button" onClick={actions.exportGeoJson}><Download size={18} />Експортувати GeoJSON</button><button className="command-button" type="button" onClick={actions.exportCsv}><FileSpreadsheet size={18} />Експортувати CSV</button></div>{canManageWorkspaces ? <WorkspaceAdminSettings actions={actions} workspace={workspace} testWorkspaceEnabled={testWorkspaceEnabled} /> : null}</section>;
}
