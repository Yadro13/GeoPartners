...
function PlotReferences({ plots, type, locale, includeSpecialized = false }: { plots: PlotFeature[]; type: PlotResultType; locale: string; includeSpecialized?: boolean }) {
  ...
  return <div className="report-matrix__references">{plots.map(({ properties }) => <span key={properties.id}><strong>{properties.cadastralNumber}</strong><small className="report-print-details">{[properties.owner, properties.lessee, formatPlotSpecialDetails(properties, type, appLocale, includeSpecialized)].filter(Boolean).join("\n")}</small></span>)}</div>;
}
