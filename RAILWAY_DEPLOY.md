...
This environment is a frozen technical deployment and is not the final customer production. The final production will be created as a separate Railway project in the customer's own Railway account on the Hobby plan. It will use HTTP transactional email delivery because Railway blocks outbound SMTP on Hobby. No staging or provisional-production resources are transferred until the customer project, domain and release window are approved.
...
Use `.env.example` only as a list of variable names for local development. Do not
commit real credentials or use a repository `.env` file as the source of Railway
secrets.

Create `EMAIL_USE_SMTP` once in **Project Settings > Shared Variables** for each
Railway environment. Share it with `web` and `notifications`; Railway creates the
following reference in both services:

```text
EMAIL_USE_SMTP=${{ shared.EMAIL_USE_SMTP }}
```

Keep both transport blocks configured in the Railway service variables. Routine
switching then requires changing only the shared value and deploying both affected
services:

- `true`: use the preconfigured SMTP block;
- `false`: use the preconfigured HTTPS API block.

Copy the remaining names from `.env.example` into the `web` service and set
environment-specific values. In particular:
...
- `EMAIL_FROM`: verified sender used by every email transport.
- SMTP mode: `SMTP_HOST`, `SMTP_PORT`, `SMTP_SECURE`, `SMTP_USER`, `SMTP_PASSWORD`.
- HTTP mode: select `EMAIL_HTTP_PROVIDER=brevo|resend` once and configure its
  `BREVO_API_KEY` or `RESEND_API_KEY`. The provider setting does not change during
  normal SMTP/API switching.
...
- `SNAPSHOT_CRON_SECRET`: one random value of at least 32 characters, shared only
  with `web` and `notifications`.
...
The `notifications` service needs `DATABASE_URL`, `APP_URL`, `ADMIN_EMAIL`, the
same email-transport variables and shared `EMAIL_USE_SMTP` reference as `web`, and
the optional Telegram variables. It also needs
`SNAPSHOT_CRON_URL=https://YOUR_DOMAIN/api/internal/snapshots/run` and the shared
`SNAPSHOT_CRON_SECRET`; its existing five-minute Railway cron calls the endpoint
only during Friday 23:00 in `Europe/Kyiv`, while the database schedule key prevents
duplicates. The `backup` service needs `DATABASE_URL` and the
six `AWS_*` variables generated by the Railway Bucket.
...
Select a folder, individual files, or up to five ZIP archives. GeoJSON is the only required source for a plot; a PDF with the same relative path and base name is optional. ZIP and folder contents enter the same review path as separately selected files. Coordinates come from GeoJSON. When a matching PDF is available, it also supplies the cadastral number, area, every owner, the complete lessee value and the document currentness date, and its original is stored in the Bucket. A cadastral mismatch remains blocking; a missing PDF does not block import and leaves unavailable fields editable by the user.

There is no user-facing 60-file limit. Large selections, including the 300-plot acceptance case, are split automatically into sequential technical requests of at most 60 files and 100 MB without separating a GeoJSON/PDF pair. A selection may contain up to 500 MB of expanded data, each extracted file remains limited to 20 MB, and a ZIP may contain up to 5,000 entries. Accepted formats are `.geojson`, `.json`, `.pdf`, and `.zip`; uploaded PDFs and ZIP archives are checked for their real file signatures. Relative paths are preserved, archive paths are validated, and unsupported files are skipped with a summary.
...
Staging on Railway Pro currently has the shared flag set to `true` and uses Brevo
SMTP:

```text
EMAIL_USE_SMTP=true
EMAIL_FROM=GeoPartners <verified-sender@example.com>
SMTP_HOST=smtp-relay.brevo.com
SMTP_PORT=587
SMTP_SECURE=false
```

Use a Brevo SMTP key as `SMTP_PASSWORD`, not the Brevo account password. A verified individual sender can be used before the final domain is selected.

The future customer production on Railway Hobby will keep the same two
preconfigured blocks, but its shared flag will be `false`. Brevo is the default
HTTPS provider because the project already uses it:
...
Resend is supported as an alternative by setting `EMAIL_HTTP_PROVIDER=resend` and `RESEND_API_KEY`. Do not set both provider keys unless an operational migration requires them. After domain selection, authenticate the sending domain with SPF, DKIM and DMARC and update `EMAIL_FROM` to the domain sender.

Changing the shared flag does not copy, generate, or overwrite credentials. Before
the first switch, verify that the selected HTTP provider key is present in both
`web` and `notifications`. The inactive block is ignored by the application.
