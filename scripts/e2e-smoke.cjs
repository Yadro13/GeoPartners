...
(async () => {
  ...
  await page.getByText("Виявлено накладання").waitFor();
  assert(await page.getByRole("button", { name: "Зберегти", exact: true }).isDisabled(), "overlap blocks plot saving");
  ...
  await page.getByText("Виявлено накладання").waitFor({ state: "detached" });
  ...
  await page.goto(`${baseUrl}?role=user`);
  await page.getByRole("button", { name: "Ділянки", exact: true }).click();
  ...
  await page.goto(`${baseUrl}?role=user&google=1`);
  await page.waitForSelector(".desktop-shell");
  ...
  await page.setViewportSize({ width: 1440, height: 900 });
  await page.goto(`${baseUrl}?google=1`);
  await page.waitForSelector(".desktop-shell");
  await page.getByTitle("Профіль").click();
  ...
  await page.goto(`${appOrigin}/reset-password`);
  ...
  await page.goto(`${appOrigin}/sign-up`);
