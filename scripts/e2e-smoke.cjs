...
(async () => {
  ...
  const editorStatusSelect = page.getByRole("dialog").locator('select[name="status"]');
  assert((await editorStatusSelect.count()) === 1 && await editorStatusSelect.isEnabled(), "editor user can assign a plot status");
  await editorStatusSelect.selectOption({ label: "отримана згода власника" });
  await page.getByRole("dialog").getByRole("button", { name: "Закрити" }).click();
  await page.getByTitle("Налаштування").evaluate((element) => element.click());
  await page.getByRole("heading", { name: "Налаштування", exact: true }).waitFor();
