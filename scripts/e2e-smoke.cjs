...
(async () => {
  ...
  const pdfLink = page.getByRole("link", { name: "Переглянути PDF" });
  await pdfLink.waitFor();
  assert((await pdfLink.getAttribute("target")) === "_blank", "plot PDF opens in a separate tab");
  assert((await page.locator(".plot-details").innerText()).includes("га"), "Ukrainian plot card uses the localized hectare abbreviation");
  assert(!(await page.locator(".plot-details").innerText()).includes(" ha"), "Ukrainian plot card does not expose the English hectare abbreviation");
  ...
  assert((await page.locator('input[name="documentActualAt"]').getAttribute("type")) === "date", "document currency is edited as a calendar date");
  assert((await page.locator('input[name="documentActualAt"]').inputValue()) === "2026-05-02", "legacy document timestamp is normalized without time");
  ...
  await page.locator('select[name="category"]').selectOption({ label: "Основний кандидат" });
  assert((await page.locator(".result-number-field").count()) === 4, "candidate exposes one compact number field for every result type");
  await page.getByLabel("Сервітут: номер").fill("7");
  assert((await page.getByText("Параметри сервітуту").count()) === 0, "candidate link does not expose final servitude parameters");
  await page.locator('select[name="category"]').selectOption({ label: "Сервітути під ЛЕП" });
  assert((await page.locator(".result-number-field").count()) === 1, "final result exposes only its own number field");
  await page.getByText("Параметри сервітуту").waitFor();
  await page.getByLabel("Сервітут: номер").fill("");
  await page.locator('select[name="category"]').selectOption({ label: "ВЕУ" });
  await page.getByLabel("ВЕУ: номер").fill("1");
  assert((await page.locator(".result-link-row__toggle").count()) === 0, "result numbers do not duplicate the input with checkboxes");
  await page.locator(".geometry-editor__neighbor").first().waitFor();
